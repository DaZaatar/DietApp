...
export function TrackerChecklist() {
  ...
  async function openAttachmentViewer(item: Item) {
    ...
    try {
      ...
      setAttachmentViewerItems(attachments);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load meal images");
                  ...
                  <li key={item.mealId} className={`rounded-xl border p-4 ${cardSurfaceClass(item.status)}`}>
                    ...
                    <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <button
                        ...
                        className="shrink-0 rounded-md border border-slate-300 px-3 py-2 text-xs font-medium text-slate-800 dark:border-slate-600 dark:text-slate-200"
                        disabled={busyMealId === item.mealId || swapBusy}
                        onClick={() => void openAttachmentViewer(item)}
                      >
                        View images
                      </button>
                    </div>
      ...
      {attachmentViewerMeal && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h3 id="attachments-dialog-title" className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              Meal images
            </h3>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
              {attachmentViewerMeal.mealType}: {attachmentViewerMeal.title}
            </p>
            {attachmentViewerLoading ? (
              <p className="mt-3 text-sm text-slate-600 dark:text-slate-400">Loading images…</p>
            ) : attachmentViewerItems.length === 0 ? (
              <p className="mt-3 text-sm text-slate-600 dark:text-slate-400">No uploaded images for this meal yet.</p>
            ) : (
